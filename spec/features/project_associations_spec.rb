...
feature 'Project Associations' do
  ...
  scenario 'allow a logged in provider to view a list of investments' do
    ...
    expect(page).to have_text(number_to_currency(investment1.original_issue, precision: 2))
    expect(page).to have_text(number_to_currency(investment1.current_outstanding, precision: 2))
    ...
    expect(page).to have_text(number_to_currency(investment2.original_issue, precision: 2))
    expect(page).to have_text(number_to_currency(investment2.current_outstanding, precision: 2))
